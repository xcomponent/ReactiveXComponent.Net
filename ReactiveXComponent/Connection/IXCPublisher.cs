...
namespace ReactiveXComponent.Connection
{
    public interface IXCPublisher : IDisposable
    {
        ...
        List<MessageEventArgs> GetSnapshot(string stateMachine, int? chunkSize, int timeout = 10000);
        Task<List<MessageEventArgs>> GetSnapshotAsync(string stateMachine, int? chunkSize, int timeout = 10000);
    }
}